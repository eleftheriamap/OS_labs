...
{	
	...
	if ((root->nr_children)==0){//Εαν η διεργασία είναι φύλλο,δηλαδή είναι αριθμός
			int num = atoi (root->name);	//Μετατροπή του string σε integer
			if ((num==0)&& strcmp(root->name,"0")!=0){	//Eαν υπάρχει σφάλμα στο αρχείο
				exit(69);
			}
			//Εγγραφή της τιμής της διεργασίας στο pipe που δημιούργησε ο πατέρας
		...
		for (i=0; i < 2; i++){
			...
			if (child == 0) {
				fork_procs(root->children+i,pfd_child); //Αναδρομή για κάθε παιδί. Ως δεύτερη παράμετρος δίνεται το pipe του πατέρα στο οποίο θα γράψει το παιδί
				exit(1);
			}
...
int main(int argc, char *argv[])
{
	...
	//Διάβασμα της τελική απάντησης από την κορυφή του δέντρου
