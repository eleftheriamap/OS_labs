...
{	
	if (WEXITSTATUS(status)==69){
		fprintf(stderr, "My PID = %ld: Child PID = %ld terminated unexpected because wrong input, exit status = %d\n",
			(long)getpid(), (long)pid, WEXITSTATUS(status));
		exit(69);
	}
	else if (WEXITSTATUS(status)==1){
		fprintf(stderr, "My PID = %ld: Child PID = %ld terminated unexpected because system error, exit status = %d\n",
			(long)getpid(), (long)pid, WEXITSTATUS(status));
		exit(1);
	}
 ...
 */
void
wait_for_dead_children(int cnt)
{
	int i;
	pid_t p;
	int status;
